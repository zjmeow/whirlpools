...
pub fn get_output<'a>(
    ...
    mut whirlpool: &Account<'a, Whirlpool>,
    token_mint_a: &InterfaceAccount<'a, Mint>,
    token_mint_b: &InterfaceAccount<'a, Mint>,
    amount: u64,
    a_to_b: bool,
) -> Result<(u64)> {
    let whirlpool = &mut whirlpool;
    let clock = Clock::get()?;
    let timestamp = to_timestamp_u64(clock.unix_timestamp)?;
    let builder = SparseSwapTickSequenceBuilder::try_from(
        whirlpool,
    ...
    let transfer_fee_excluded_output_amount = if a_to_b {
        calculate_transfer_fee_excluded_amount(&token_mint_b, swap_update.amount_b)?.amount
    } else {
        calculate_transfer_fee_excluded_amount(&token_mint_a, swap_update.amount_a)?.amount
    };
